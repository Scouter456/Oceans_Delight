...
public class BlockstateGenerator extends BlockStateProvider {
    ...
    public BlockstateGenerator(PackOutput output, ExistingFileHelper exFileHelper) {
        super(output, OceansDelight.MODID, exFileHelper);
    }
    ...
    public ResourceLocation resourceBlock(String path) {
        return ResourceLocation.fromNamespaceAndPath(OceansDelight.MODID, "block/" + path);
    }
    ...
    public void tomatoBlock(Block block, String baseName, ResourceLocation texture, String  renderType) {
        ModelFile age1 = models().cross(baseName + "_stage0",  ResourceLocation.parse(texture.toString() + "_stage0")).renderType(renderType);
        ModelFile age2 = models().cross(baseName + "_stage1",  ResourceLocation.parse(texture.toString() + "_stage1")).renderType(renderType);
        ModelFile age3 = models().cross(baseName + "_stage2",  ResourceLocation.parse(texture.toString() + "_stage2")).renderType(renderType);
        ModelFile age4 = models().cross(baseName + "_stage3",  ResourceLocation.parse(texture.toString() + "_stage3")).renderType(renderType);
